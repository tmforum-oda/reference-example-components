...
//
// Validates each engagedParty entry that has an href by issuing an HTTP GET.
// Works for both PartyRef (TMF632) and PartyRoleRef (TMF669) — the href itself
// encodes which service to call; no @type discrimination needed.
//
async function validateEngagedPartyHref(payload) {
  const items = payload?.engagedParty;
  ...
  for (const item of items) {
    ...
    try {
      ...
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY,
        `Could not resolve engagedParty.href: ${href}`);
    }
