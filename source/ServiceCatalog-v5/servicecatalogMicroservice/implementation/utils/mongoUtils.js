...
function getMongoQuery(req) {
  ...
  // query-to-mongo converts bare ISO date strings to Date objects, but dates are
  // stored as ISO strings in MongoDB — convert back to regex for string matching
  Object.keys(res.criteria).forEach(prop => {
    const val = res.criteria[prop];
    if (val instanceof Date) {
      res.criteria[prop] = {$regex: '^' + val.toISOString()};
    }
  });
