...
//
// validateRequest — checks the payload against validationRulesType2 rules.
// In v5 (express-openapi-validator), schema validation is handled by the middleware,
// so this is only needed for business rules beyond the spec (e.g. "orderItem must have action").
// Call this from a service override if custom payload rules are needed.
//
// Usage: validateRequest('create{Resource}', payload, 'POST')
//
function validateRequest(operation, doc, method) {
  return new Promise(function(resolve, reject) {
    method = (method || 'POST').toUpperCase();

    // derive type from operation name — strip 'create', 'patch', 'list', 'retrieve', 'delete'
...
// Validates serviceQualificationItem[*].service.serviceSpecification.href against TMF633 (ServiceCatalog).
// Iterates every item in the array; skips items where serviceSpecification or its href is absent.
async function validateServiceSpecificationHref(payload) {
  const items = payload?.serviceQualificationItem;
  if (!items || !Array.isArray(items)) return payload;
  for (const item of items) {
    const href = item?.service?.serviceSpecification?.href;
    if (!href) continue;
    try {
      ...
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY,
        `Could not resolve serviceSpecification.href: ${href}`);
    }
  }
  return payload;
}

module.exports = { validateRequest, validateServiceSpecificationHref };
