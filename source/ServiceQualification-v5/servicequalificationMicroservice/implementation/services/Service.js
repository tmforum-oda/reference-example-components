  ...
  static addMandatoryFilterFields(query, resourceType) {
    ...
    if (projection && Object.keys(projection).length > 0) {
      ...
      if (Object.keys(projection).includes('none')) {
        // ?fields=none: strip everything except the bare mandatory set
      ...
      // Split the projection into real inclusions and !-prefixed exclusions, then
      // rebuild: pure-exclusion mode uses a MongoDB exclusion projection (no mixing);
      // inclusion mode merges with default/required fields (exclusions are naturally
      // absent because unlisted fields don't appear in an inclusion result).
      const exclusions = {};
      const inclusions = {};
