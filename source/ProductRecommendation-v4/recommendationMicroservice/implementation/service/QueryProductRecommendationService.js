...
const {validateRequest, validateCategoryHref} = require('../utils/ruleUtils');
...
exports.createQueryProductRecommendation = function(req, res, next) {
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);
  const internalError = new TError(TErrorEnum.INTERNAL_SERVER_ERROR, "Internal database error");
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createQueryProductRecommendation', payload))
    .then(payload => traverse(req, requestSchema, payload, [], getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createQueryProductRecommendation', payload))
    .then(payload => validateCategoryHref(payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        ...
        }).catch(error => { console.log("createQueryProductRecommendation: error=" + error); sendError(res, internalError); });
      }).catch(error => sendError(res, internalError));
    }).catch(error => sendError(res, error));
};

exports.listQueryProductRecommendation = function(req, res, next) {
  const resourceType = getResponseType(req);
  const internalError = new TError(TErrorEnum.INTERNAL_SERVER_ERROR, "Internal database error");
...
exports.retrieveQueryProductRecommendation = function(req, res, next) {
  const resourceType = getResponseType(req);
  const internalError = new TError(TErrorEnum.INTERNAL_SERVER_ERROR, "Internal database error");
  var query = mongoUtils.getMongoQuery(req);
