...
// Validates service.serviceSpecification.href against TMF633 (ServiceCatalog).
// Skips validation if serviceSpecification or serviceSpecification.href is absent.
async function validateServiceSpecificationHref(payload) {
  const href = payload?.serviceSpecification?.href;
  ...
  try {
    ...
    throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve serviceSpecification.href: ${href}`);
  }
  return payload;
}
