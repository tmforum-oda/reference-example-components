...
docker buildx build -t "akumartmf/serviceinventoryapiv5:0.2" --platform "linux/amd64,linux/arm64" -f serviceinventory-dockerfile . --push
...
docker buildx build -t "akumartmf/serviceinventorymcp:0.4" --platform "linux/amd64,linux/arm64" -f serviceinventory-mcp-dockerfile . --push
