...
if (!componentName) { componentName = 'r1-servicecatalogmanagement'; }
...
swaggerTools.initializeMiddleware(swaggerDoc, function(middleware) {
  ...
  app.use(function(err, req, res, next) {
    if (err && err.failedValidation) {
      const e = new TError(TErrorEnum.INVALID_FIELD_VALUE, err.message);
      sendError(res, e);
    } else { next(err); }
  });
  ...
  http.createServer(app).listen(serverPort, function() {
    console.log('Service Quality Management API listening on port %d', serverPort);
  });
});
