...
const {validateRequest, validateUserHref} = require('../utils/ruleUtils');
...
exports.createPermission = function(req, res, next) {
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createPermission', payload))
    .then(payload => traverse(req, requestSchema, payload, [], getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createPermission', payload))
    .then(payload => validateUserHref(payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        ...
        }).catch(error => { console.log("createPermission: error=" + error); sendError(res, internalError); });
      }).catch(error => sendError(res, internalError));
    }).catch(error => sendError(res, error));
};

exports.listPermission = function(req, res, next) {
  const resourceType = getResponseType(req);
  const internalError = new TError(TErrorEnum.INTERNAL_SERVER_ERROR, "Internal database error");
...
exports.retrievePermission = function(req, res, next) {
  const resourceType = getResponseType(req);
  const internalError = new TError(TErrorEnum.INTERNAL_SERVER_ERROR, "Internal database error");
  var query = mongoUtils.getMongoQuery(req);
...
exports.patchPermission = function(req, res, next) {
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);
  const internalError = new TError(TErrorEnum.INTERNAL_SERVER_ERROR, "Internal database error");
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'patchPermission', payload))
    .then(payload => traverse(req, requestSchema, payload, [], getPayloadType(req)))
    .then(payload => {
      mongoUtils.connect().then(db => {
        const id = req.swagger.params['id'].value;
