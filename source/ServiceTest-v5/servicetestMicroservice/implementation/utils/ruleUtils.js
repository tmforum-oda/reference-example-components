...
// Pattern A — validates relatedService.href (scalar) against TMF638 (POST /serviceTest)
async function validateRelatedServiceHref(payload) {
  const href = payload?.relatedService?.href;
  if (!href) return payload;
  let parsedUrl;
  try {
    parsedUrl = new URL(href);
  } catch (e) {
    throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve relatedService.href: ${href}`);
  }
...
module.exports = { validateRequest, validateRelatedServiceHref };
