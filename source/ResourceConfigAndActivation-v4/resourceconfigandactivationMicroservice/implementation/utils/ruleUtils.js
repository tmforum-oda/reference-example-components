...
// Pattern A — validates resource.resourceSpecification.href (scalar) against TMF634
async function validateResourceSpecificationHref(payload) {
  ...
  try {
    ...
    throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve resourceSpecification.href: ${href}`);
  }
  return payload;
}

// Pattern B — validates resource.resourceRelationship[*].resource.href (array) against TMF639
async function validateResourceRelationshipHref(payload) {
  const relationships = payload?.resourceRelationship;
  if (!Array.isArray(relationships) || relationships.length === 0) return payload;
  for (const rel of relationships) {
    const href = rel?.resource?.href;
    if (!href) continue;
    try {
      await axios.get(href, {
        httpsAgent: new https.Agent({ rejectUnauthorized: false }),
      ...
      if (err?.code !== undefined && err?.reason !== undefined) throw err;
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve resourceRelationship[].resource.href: ${href}`);
    }
  }
  return payload;
}
...
function processRules(rules, obj) {
  ...
  if (Array.isArray(rules)) {
    rules.forEach(rule => {
      ...
      error = error.concat(processRule(rule, obj, path));
    });
  }
  return error;
}
...
module.exports = { validateRequest, validateResourceSpecificationHref, validateResourceRelationshipHref };
