...
const {validateRequest, validateResourceSpecificationHref, validateResourceRelationshipHref} = require('../utils/ruleUtils');
...
exports.createResource = function(req, res, next) {
  ...
  swaggerUtils.getPayload(req)
    ...
    .then(payload => validateResourceRelationshipHref(payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        ...
        }).catch(error => { console.log("createResource: error=" + error); sendError(res, internalError); });
...
exports.patchResource = function(req, res, next) {
  ...
  swaggerUtils.getPayload(req)
    ...
    .then(payload => traverse(req, requestSchema, payload, [], getPayloadType(req)))
    .then(payload => processAssignmentRules('patchResource', payload))
    .then(payload => {
      payload = swaggerUtils.updatePayloadServiceType(payload, req, '');
      mongoUtils.connect().then(db => {
        db.collection(resourceType).findOneAndUpdate({ id: id }, { $set: payload }, { returnOriginal: false }).then(doc => {
          if (!doc.value) {
            sendError(res, new TError(TErrorEnum.RESOURCE_NOT_FOUND, "No resource with given id found"));
          } else {
            doc = cleanPayloadServiceType(doc.value);
            sendDoc(res, 200, doc);
            notificationUtils.publish(req, doc);
          }
        }).catch(error => { console.log("patchResource: error=" + error); sendError(res, internalError); });
      }).catch(error => sendError(res, internalError));
    }).catch(error => sendError(res, error));
};
...
exports.deleteResource = function(req, res, next) {
  ...
  const id = req.swagger.params['id'].value;
  mongoUtils.connect().then(db => {
    db.collection(resourceType).deleteOne({ id: id }).then(result => {
    ...
    }).catch(error => { console.log("deleteResource: error=" + error); sendError(res, internalError); });
  }).catch(error => sendError(res, internalError));
};
