...
// Validates relatedParty[*].href against TMF669 for entries where @referredType === 'partyRole'.
// Skips validation if relatedParty is absent or if a specific entry has no href.
async function validateRelatedPartyHref(payload) {
  const relatedParty = payload?.relatedParty;
  if (!relatedParty || !Array.isArray(relatedParty)) return payload;

  for (const party of relatedParty) {
    if (party?.['@referredType'] !== 'partyRole') continue;
    const href = party?.href;
    if (!href) continue;
    try {
...
module.exports = { validateRequest, validateRelatedPartyHref };
