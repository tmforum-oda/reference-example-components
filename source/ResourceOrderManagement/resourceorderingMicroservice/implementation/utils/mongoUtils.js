...
function getMongoQuery(req) {
  ...
  //
  try {
    const requestType = getResponseType(req);
    ...
    properties.forEach(prop => {
      ...
      if(paramDef!==undefined && paramDef.type === "string" && paramDef.format === "date-time") {
        ...
        if(propVal instanceof Date) {
          // query-to-mongo converts ISO strings to Date objects, but values are
          // stored as ISO strings — convert back so the regex matches the stored string
          res.criteria[prop] = {$regex: '^' + propVal.toISOString() + '.*'};
