...
function checkPresentIf(criteria,obj,path,lastLabel) {
  ...
  if(criteria.$presentIf) {
    if(! hasProperty(obj,criteria.$presentIf)) {
      const res = path + ":" + JSON.stringify(criteria);
      error.push(res);
    };
  };
...
// Validator: check resourceSpecification.href on each orderItem (against TMF634)
async function validateResourceSpecificationHref(payload) {
  const items = payload && payload.orderItem;
  if (!items || !Array.isArray(items)) return payload;

  for (const item of items) {
    const href = item && item.resourceSpecification && item.resourceSpecification.href;
    if (!href) continue;
    try {
      ...
      if (err && err.code !== undefined && err.reason !== undefined) throw err;
...
// Validator: check resource.href on each orderItem (against TMF639)
async function validateResourceHref(payload) {
  const items = payload && payload.orderItem;
  if (!items || !Array.isArray(items)) return payload;

  for (const item of items) {
    const href = item && item.resource && item.resource.href;
    if (!href) continue;
    try {
      await axios.get(href, {
        httpsAgent: new https.Agent({ rejectUnauthorized: false }),
      ...
      if (err && err.code !== undefined && err.reason !== undefined) throw err;
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, 'Could not resolve resource.href: ' + href);
    }
  }
  return payload;
}

module.exports = { validateRequest, validateResourceSpecificationHref, validateResourceHref };
