...
docker buildx build -t "lesterthomas/productcatalogapi:0.7"  --platform "linux/amd64,linux/arm64" -f prodcat-dockerfile . --push
...
docker buildx build -t "lesterthomas/roleinitialization:0.2"  --platform "linux/amd64,linux/arm64" -f roleinitialization-dockerfile . --push
