# Service Inventory Source Code

This is the Node.js reference implementation of the [TMFC008 Service Inventory Management](https://www.tmforum.org/oda/directory/components-map/production/TMFC008) ODA Component. For deployment instructions see the Helm chart at [../../charts/ServiceInventory](/charts/ServiceInventory/).

## Repository structure

| Directory / File | Description |
|---|---|
| `serviceInventoryMicroservice/` | Node.js implementation of the TMF638 Service Inventory Management Open API |
| `roleInitializationMicroservice/` | Bootstraps the initial `canvasRole` on first deploy |
| `serviceInventoryInitializationMicroservice/` | Registers the metrics microservice as a hub listener on first deploy |
| `openMetricsMicroservice/` | Prometheus/OpenMetrics endpoint that counts business events |
| `serviceinventory-dockerfile` | Dockerfile for the Service Inventory API microservice |
| `roleinitialization-dockerfile` | Dockerfile for the role initialization job |
| `serviceinventoryinitialization-dockerfile` | Dockerfile for the initialization job |
...
1. **serviceInventoryMicroservice** is the main API server. It receives REST calls, stores `Service` documents in MongoDB, and publishes events to all registered listeners via `notificationUtils.publish()` on every create/patch/delete operation.

2. **serviceInventoryInitializationMicroservice** runs once as a Kubernetes Job on startup. It POSTs to the Service Inventory hub endpoint (`/tmf-api/serviceInventory/v4/hub`) to register the metrics microservice as a listener. After registration it exits.

3. **openMetricsMicroservice** listens on `/listener` (POST) for events from the Service Inventory API. Each event increments a Prometheus counter (`{release}_serviceinventory_api_counter`). It also serves metrics at `/{componentName}/metrics` for Prometheus scraping.
...
5. **MongoDB** is a shared document store used by the Service Inventory API and the role management microservice.

```
[Client] ──REST──► [serviceInventoryMicroservice] ──► [MongoDB]
                            │
                            │ publish events
                            ▼
                    [openMetricsMicroservice]
                    (registered via hub at startup by
                     serviceInventoryInitializationMicroservice)
```

## Main API microservice

`serviceInventoryMicroservice/implementation/` contains:
...
| `api/swagger.yaml` | TMF638 OpenAPI spec — defines all routes, schemas, and `x-swagger-router-controller` annotations |
| `controllers/ServiceInventory.js` | Thin passthrough — maps operationIds to `ServiceInventoryService` |
...
| `service/ServiceInventoryService.js` | MongoDB CRUD for Service — publishes events on create/patch/delete |
...
From the `source/ServiceInventory/` directory:
...
```bash
docker buildx build -t "lesterthomas/serviceinventoryapi:0.1" --platform "linux/amd64,linux/arm64" -f serviceinventory-dockerfile . --push
...
To run the Service Inventory API locally against a local MongoDB:

```bash
cd serviceInventoryMicroservice/implementation
npm install
MONGODB_HOST=localhost MONGODB_PORT=27017 MONGODB_DATABASE=tmf COMPONENT_NAME=r1-serviceinventory node index.js
```

The API will be available at `http://localhost:8080/r1-serviceinventory/tmf-api/serviceInventory/v4/` with Swagger UI at `/r1-serviceinventory/tmf-api/serviceInventory/v4/docs`.
...
5. Upgrade the Helm release (`helm upgrade r1 ../../charts/ServiceInventory -n components`).
