...
docker buildx build -t "lesterthomas/serviceinventoryapi:1.0"  --platform "linux/amd64,linux/arm64" -f serviceinventory-dockerfile . --push
...
docker buildx build -t "lesterthomas/serviceinventorymetrics:1.0" --platform "linux/amd64,linux/arm64" -f openMetricsMicroservice-dockerfile . --push
