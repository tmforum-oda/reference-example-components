...
if (!componentName) {
  componentName = 'r1-productordercaptureandvalidation' // for local testing, if not set
}
...
function errorHandler(err, req, res, next) {
  if (err) {
    console.log('errorHandler err=' + JSON.stringify(err));
    const error = new TError(TErrorEnum.UNPROCESSABLE_ENTITY, err.message || 'Validation error');
    sendError(res, error);
  } else {
    next();
  }
