...
function generateQueryString(query, offset, limit) {
  ...
  if (query?.options?.projection) {
    ...
    if (fields.length > 0) res.push('fields=' + fields.join(','));
  }

  if (query?.criteria) {
    let criteria = Object.keys(query.criteria);
    if (criteria.length > 0) {
      res.push(criteria.map(c => c + '=' + query.criteria[c]).join(','));
    }
  }

  if (query?.jsonpath) res.push('filter=' + query.jsonpath);
  if (offset > 0) res.push('offset=' + offset);
  if (limit > 0) res.push('limit=' + limit);

  return res.length == 0 ? '' : '?' + res.join('&');
}

module.exports = { generateResponseHeaders };
