...
// Pattern B — validates relatedProductSpecification[*].href against TMF620 (POST /productTestSpecification)
async function validateRelatedProductSpecificationHref(payload) {
  const items = payload?.relatedProductSpecification;
  ...
  for (const item of items) {
    ...
    try {
      ...
      throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve relatedProductSpecification.href: ${href}`);
    }
...
// Pattern A — validates relatedProduct.href (scalar) against TMF637 (POST /productTest)
async function validateRelatedProductHref(payload) {
  const href = payload?.relatedProduct?.href;
  if (!href) return payload;
  let parsedUrl;
  try {
    parsedUrl = new URL(href);
  } catch (e) {
    throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve relatedProduct.href: ${href}`);
  }
  if (parsedUrl.protocol !== 'http:' && parsedUrl.protocol !== 'https:') {
    throw new TError(TErrorEnum.UNPROCESSABLE_ENTITY, `Could not resolve relatedProduct.href: ${href}`);
  }
...
module.exports = { validateRequest, validateRelatedProductSpecificationHref, validateRelatedProductHref };
