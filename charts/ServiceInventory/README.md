# Example Service Inventory component

This is an example implementation of a [TM Forum Service Inventory Management](https://www.tmforum.org/oda/directory/components-map/production/TMFC008) component (TMFC008).

This folder is the Helm Chart package which you distribute or host in a Helm Chart Repository. The source code is available at [../../source/ServiceInventory](/source/ServiceInventory/). The source README contains all the implementation documentation.
...
* The *mandatory* TMF638 Service Inventory Management Open API, providing CRUD operations for `Service` resources plus a hub endpoint for event subscriptions.

### Management function

In its **management function** it implements:
* An open metrics endpoint that counts business events (Service create/update/delete). The Prometheus query to graph service creation events is:
  ```
  rate(si_serviceinventory_api_counter{NotificationEvent="ServiceCreateEvent"}[5m])
  ```
...
* **serviceInventoryMicroservice** — implements the TMF638 Service Inventory Management Open API (Service resources).
...
* **openMetricsMicroservice** — implements the open metrics API, counts business events published by the Service Inventory API.
* **serviceInventoryInitializationMicroservice** — registers the metrics microservice as a listener on the Service Inventory hub. Deployed as a Kubernetes Job that runs once on initialisation.
...
```
helm install r1 .\serviceinventory -n components
```
...
```
helm install r1 .\serviceinventory --set permissionspec.enabled=false -n components
```
...
| `api.image` | `lesterthomas/serviceinventoryapi:0.1` | Docker image for the Service Inventory API microservice |
...
| `metrics.image` | `lesterthomas/serviceinventorymetrics:0.1` | Docker image for the open metrics microservice |
